...
**CI fix: `next lint` had no ESLint config and was silently hanging on an interactive prompt in CI since the pipeline was added (2026-07-03)** — `npm run type-check` always passed, masking that the lint half of every CI run since then failed with an unhelpful "Process completed with exit code 1" and zero actual lint output. Added `eslint.config.mjs` (flat config, `next/core-web-vitals` + `next/typescript`, no new dependency). This now surfaces 56 real pre-existing lint errors + 5 warnings across ~19 files unrelated to tonight's changes — deferred as a dedicated cleanup pass rather than bulk-editing unrelated files in an unattended run; fixed only the 3 violations in files already touched tonight (a dead `ScoreField` type in `HistoryClient.tsx`, and two Next.js route handlers' structurally-required-but-unused `_req` params).
- **Files**: `eslint.config.mjs` (new), `app/api/pt/avatar/route.ts`, `app/api/pt/logo/route.ts`, `components/client/HistoryClient.tsx`.
